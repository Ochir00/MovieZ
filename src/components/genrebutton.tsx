    ...
    <Card className="w-[97px] h-[36px] flex justify-center items-center rounded-[6px]">
      <Popover>
        <PopoverTrigger className="flex">
          ...
          <p className="pt-0.5 text-[14px] font-black">Genre</p>
        </PopoverTrigger>
        <PopoverContent className="w-[537px] ml-[438px] mt-[5px]">
          ...
          <ToggleGroup
            className="w-[100%] flex gap-4 flex-wrap mt-[15px] justify-start "
            type="multiple"
          >
            {data.genres.map((props: zgrlutga, key: number) => {
              return (
                <Link href={`/genres?genresId=${props.id}`} key={key}>
                  {" "}
                  <ToggleGroupItem
                    key={key}
                    value={props.id.toString()}
                    className="border rounded-[9px]  px-[10px] text-[12px] font-semibold flex items-center  gap-[5px]"
                  >
                    {props.name}
